...
from binance import ThreadedWebsocketManager  # python-binance: сам держит и реконнектит WS
...
BREAKOUT_PCT         = 3.0    # (high-open)/open ≥ 3% = выход вверх из сжатия.
                              # 3% — ранняя реакция из пружины (откат+памп ещё впереди).
                              # Считаем от открытия, не размах: убирает шум фитилей.
...
# Менеджер сам держит соединение, отвечает на ping и реконнектит.
# Дробим подписку на сокеты по STREAMS_PER_CONN, чтобы один сокет не вёз всё.
STREAMS_PER_CONN     = 150    # потоков на один сокет (~4 сокета на 587 монет)
WS_QUEUE_SIZE        = 5000   # размер очереди сообщений менеджера. Дефолт 100
                              # переполнялся на старте (587 потоков = шквал) →
                              # QueueOverflow и шторм реконнектов. 5000 = с запасом.
...
twm = None  # ThreadedWebsocketManager — создаётся в start_websockets
...
def check_spring(symbol: str):
    ...
    # Выход ≥ порога? Считаем движение ВВЕРХ ОТ ОТКРЫТИЯ (high-open)/open.
    # Так на чистой пружине (open внизу свечи) ловим точно, а шум от свечей
    # с нижним фитилём (качание вниз-вверх в коридоре) отсекается —
    # подтверждено на реальных сигналах FLNC/SKYAI: у них фитиля снизу не было.
...
def build_alert_message(symbol: str, det: dict) -> str:
    ...
    lines = [
        ...
        f"💥 Low: <code>{fmt_price(det['low'])}</code>",
...
# ─────────────────────────────────────────────
# ОБРАБОТКА WS-СООБЩЕНИЙ (callback python-binance)
# ─────────────────────────────────────────────
def handle_socket_message(msg):
    """
    Callback от ThreadedWebsocketManager (futures multiplex).
    Формат: {'stream': 'btcusdt@kline_1h', 'data': {'e':'kline','k':{...}}}
    либо при ошибке: {'e': 'error', 'm': '...'}.
    python-binance сам держит соединение, шлёт pong и реконнектит.
    """
    ...
    try:
        ...
        if isinstance(msg, dict) and msg.get('e') == 'error':
            logging.error(f"WS error от менеджера: {msg.get('m')}")
            bot_status['ws_reconnects'] += 1
            return

        # Достаём data из multiplex-обёртки
        ...
        stream_sym = k['s'].lower()
        symbol = symbol_map.get(stream_sym)
        if not symbol:
            return
...
# ─────────────────────────────────────────────
# ЗАПУСК WS через python-binance ThreadedWebsocketManager
# ─────────────────────────────────────────────
def build_stream_names(perps: list) -> list:
    """
    Имена стримов из ТОЧНОГО id рынка ccxt (market['id'] → 'btcusdt' и т.п.),
    заполняет symbol_map. Возвращает список '<sym>@kline_<tf>'.
    """
    streams = []
    for symbol in perps:
...
def start_websockets(perps: list):
    """
    Поднимает WS через ThreadedWebsocketManager. Библиотека сама держит
    соединение, отвечает на ping и реконнектит. Подписку дробим на пачки
    по STREAMS_PER_CONN отдельными сокетами, чтобы не перегружать один.
    """
    global twm
    streams = build_stream_names(perps)
    chunks = [streams[i:i + STREAMS_PER_CONN]
              for i in range(0, len(streams), STREAMS_PER_CONN)]
    logging.info(f"Всего стримов: {len(streams)}, сокетов: {len(chunks)}")

    twm = ThreadedWebsocketManager(max_queue_size=WS_QUEUE_SIZE)
    twm.start()  # запускает фоновый цикл менеджера
    ...
    for i, chunk in enumerate(chunks):
        try:
            ...
            logging.info(f"WS-сокет #{i}: подписка на {len(chunk)} стримов")
        except Exception as e:
            logging.error(f"WS-сокет #{i} ошибка запуска: {e}")
        time.sleep(2)  # разносим старты сокетов
...
def watchdog():
    """
    Следит за потоком WS-данных. python-binance реконнектит сам, но если
    данные не идут долго (менеджер завис) — перезапускаем его целиком.
    Даёт запас на старт (история + подписка).
    """
    time.sleep(300)
    ...
    while True:
        ...
        if current == last_count:
            stalls += 1
            logging.error(f"WATCHDOG: WS-данные не идут 2 мин (подряд {stalls})")
            # После двух пустых проверок подряд — безопасный перезапуск
            if stalls >= 2:
                restart_websockets()
                stalls = 0
        else:
...
def restart_websockets():
    """
    Безопасный перезапуск WS-менеджера: гарантированно гасим старый,
    ждём, создаём новый. Раньше watchdog создавал новый менеджер поверх
    живого → 'failed to initialize' и зависание всего бота.
    """
    global twm
    logging.error("WATCHDOG: безопасный перезапуск WS-менеджера")
    old = twm
    twm = None
    if old is not None:
        try:
            ...
            logging.warning(f"WATCHDOG: stop старого менеджера: {e}")
    # Даём старому менеджеру и его потокам реально завершиться
    time.sleep(10)
    try:
        perps = get_all_usdt_perps()
        start_websockets(perps)
        logging.info("WATCHDOG: WS-менеджер перезапущен")
    except Exception as e:
        logging.error(f"WATCHDOG: перезапуск не удался: {e}")
