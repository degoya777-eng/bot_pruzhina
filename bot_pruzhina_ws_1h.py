...
STREAMS_PER_CONN     = 1000   # лимит Binance 1024 на соединение; берём с запасом
SUBSCRIBE_VIA_URL    = True   # подписка прямо в URL (без лимита 5 msg/s на subscribe)
...
def on_ws_open(ws):
    logging.info("WS соединение открыто")
...
def run_ws_connection(stream_chunk: list, conn_id: int):
    ...
    while True:
        try:
            ...
            ws_apps.append(ws)
            logging.info(f"WS#{conn_id}: подключение к {len(stream_chunk)} стримам")
            # ping_interval < 10 мин (Binance шлёт ping каждые 3 мин);
            # run_forever сам отвечает pong и переподключается при разрыве
            ws.run_forever(ping_interval=180, ping_timeout=30)
        except Exception as e:
            logging.error(f"WS#{conn_id} исключение: {e}")

        # Сюда попадаем при разрыве — реконнект
        bot_status['ws_reconnects'] += 1
        logging.warning(f"WS#{conn_id}: разрыв, реконнект через 5с...")
        time.sleep(5)
...
def start_websockets(perps: list):
    ...
    # Разбиваем на пачки по STREAMS_PER_CONN (лимит Binance 1024)
    ...
    for i, chunk in enumerate(chunks):
        ...
        t.start()
        time.sleep(1)  # разносим старты соединений
...
def watchdog():
    """
    Если WS-сообщения перестали приходить — что-то зависло.
    Логируем тревогу (реконнект уже встроен в run_ws_connection).
    """
    time.sleep(120)
    ...
    while True:
        ...
        if current == last_count:
            logging.error("WATCHDOG: WS-сообщения не приходят 2 мин — поток мог зависнуть")
            bot_status['ws_reconnects'] += 1
        last_count = current
