...
BREAKOUT_PCT         = 3.0    # размах свечи (high-low)/low ≥ 3% = выход из сжатия.
...
def check_spring(symbol: str):
    ...
    # Выход ≥ порога? Для пружины считаем ПОЛНЫЙ размах свечи (high-low)/low,
    # а не от открытия — ловим амплитуду движения целиком.
    breakout_pct = (high_p - low_p) / low_p * 100
...
def build_alert_message(symbol: str, det: dict) -> str:
    title = f"💎 ПРУЖИНА 1H — размах +{det['breakout_pct']:.2f}%"
