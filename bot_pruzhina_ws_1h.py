...
STREAMS_PER_CONN     = 1000   # лимит Binance 1024 стрима на соединение
SUB_BATCH            = 100    # стримов в одном SUBSCRIBE (лимит 5 msg/s — шлём с паузой)
SUB_PAUSE            = 0.5    # пауза между батчами SUBSCRIBE
...
def run_ws_connection(stream_chunk: list, conn_id: int):
    ...
    while True:
        ...
        try:
            ...
            # обновляем ссылку для watchdog (не накапливаем — заменяем)
            while conn_id >= len(ws_apps):
                ws_apps.append(None)
            ws_apps[conn_id] = ws
            logging.info(f"WS#{conn_id}: подключение ({len(stream_chunk)} стримов)")
            # ping_interval > серверного (180с): не шлём свои частые ping
